...
async function main() {
  const db = getAdminDb();
  await db.collection('alignments').doc('orach_chayim_10').delete();

  const result = await getOrBuildSimanAlignment('Orach Chayim', 10);
  if (!result) {
    console.error('Failed to build alignment.');
    return;
  }

  let output = '# Alignment Results for Orach Chayim 10\n\n';
