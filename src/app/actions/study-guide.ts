...
/**
 * Interface for demo guide data displayed in my-guides for free users.
 */
...
/**
 * Load all 3 demo seifim (א:א, א:ב, א:ג) from canonical cache for display.
 * Returns only cached content - does not generate new content.
 */
export async function loadDemoSeifim(): Promise<DemoGuideData[]> {
  const defaultSources: SourceKey[] = ['shulchan_arukh', 'mishnah_berurah'];
  const demoSeifim: DemoGuideData[] = [];

  for (const seif of ['1', '2', '3']) {
    const request: MultiSourceRequest = {
    ...
    const cacheKey = buildCanonicalCacheKey(request);
    const cached = await loadCanonicalGuide(cacheKey);
    ...
    if (cached) {
      demoSeifim.push({
        ...
        chunks: cached.sourceResults.flatMap(sr =>
          sr.chunks.map(c => ({
            sourceKey: sr.sourceKey,
            orderIndex: c.orderIndex,
            rawText: c.rawText,
            explanationText: c.explanation,
          }))
        ),
      });
    }
...
/**
 * Check if a guide already exists in the canonical cache or user's collection.
 * Returns the cached data if found, so we don't regenerate.
 */
export async function checkExistingGuide(
  request: MultiSourceRequest,
): Promise<{ exists: boolean; guideData?: GenerationResult['guideData'] }> {
  // First check canonical cache
  const canonicalCacheKey = buildCanonicalCacheKey(request);
  const cached = await loadCanonicalGuide(canonicalCacheKey);

  if (cached) {
    return { exists: true, guideData: cached };
  }

  // If user is authenticated, also check their personal collection
  try {
    const authUser = await getAuthenticatedUser();
    const db = getAdminDb();
    // Build tref: "Orach Chayim 1:2" format
    const tref = request.seif
      ? `Shulchan Arukh, ${request.section} ${request.siman}:${request.seif}`
      : `Shulchan Arukh, ${request.section} ${request.siman}`;

    const userGuidesSnap = await db
      .collection('users')
      .doc(authUser.uid)
    ...
    if (!userGuidesSnap.empty) {
      const guideDoc = userGuidesSnap.docs[0];
      const guideData = guideDoc.data();

      if (guideData.status === 'Published' || guideData.status === 'Preview') {
        // Load chunks
        const chunksSnap = await guideDoc.ref.collection('textChunks').orderBy('orderIndex').get();
        const chunks = chunksSnap.docs.map(d => d.data());

        // Group chunks by source
        const sourceMap = new Map<string, ProcessedChunk[]>();
        for (const chunk of chunks) {
          const key = chunk.sourceKey as string;
          if (!sourceMap.has(key)) sourceMap.set(key, []);
          sourceMap.get(key)!.push({
            id: chunk.id as string || `chunk-${chunk.orderIndex}`,
            orderIndex: chunk.orderIndex as number,
            rawText: chunk.rawText as string,
            explanation: chunk.explanationText as string,
            rawHash: '',
            cacheHit: true,
          });
        }

        const sourceResults: SourceResult[] = Array.from(sourceMap.entries()).map(([sourceKey, sourceChunks]) => ({
          sourceKey: sourceKey as SourceKey,
          hebrewLabel: SOURCE_CONFIGS[sourceKey as SourceKey]?.hebrewLabel || sourceKey,
          tref: guideData.tref,
          chunks: sourceChunks,
        }));
    ...
    // Not authenticated or error - continue
  }
...
/**
 * Returns the number of guides generated by the current user this calendar month,
 * along with their monthly generation limit.
 */
export async function getMonthlyUsageCount(): Promise<{ count: number; limit: number }> {
  try {
    const authUser = await getAuthenticatedUser();
    const db = getAdminDb();
    const now = new Date();
    ...
    const snap = await db
      ...
      .get();
    const usagePolicy = await getUserUsagePolicy({ uid: authUser.uid, email: authUser.email });
    return { count: snap.size, limit: usagePolicy.monthlyGenerationLimit ?? 30 };
  } catch {
