...
`
  ...
  const mbLine = hasMb
    ? `- משנה ברורה: פרט כל פוסק המוזכר במשנה ברורה לנושא זה – שמו ודינו בקצרה (כולל ביאור הלכה, שיטת הרא"ש ואחרונים שמוזכרים).
`
    : '';
...
## <u>שם הנושא</u>
- עיקרי הדעות (ראשונים ואחרונים), מקובצות לפי שיטה. אם אין מחלוקת, כתוב זאת.
${saLine}${mbLine}- הלכה: פסיקת השולחן ערוך בשורה אחת ברורה ומעשית.
- רב עובדיה יוסף: פסיקתו של הרב עובדיה יוסף זצ"ל בנושא זה. אם לא ידועה הכרעה ספציפית, כתוב "אין הכרעה ידועה בנושא זה".
- דוגמאות מעשיות: 2–3 דוגמאות קונקרטיות מחיי היום-יום המודרני שבהן דין זה רלוונטי.
