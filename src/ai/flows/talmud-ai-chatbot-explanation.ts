...
${input.sourceKey !== 'beit_yosef'
  ? '3. **פירוק וסיכום שיטות (לטור בלבד):** כאשר ישנה מחלוקת או שמובאות כמה דעות, עשה סדר! הסבר כל שיטה בנפרד (מי הפוסק, מה הדין שלו, ומה טעמו). אם יש צורך, הוסף משפט סיכום קצר המחדד את ההבדל בין השיטות.'
  : '3. **סגנון זורם בלבד (לבית יוסף):** שמור על ביאור משולב ורציף בלבד — אין לפרק את הטקסט לשיטות בנפרד. הסדר בין הדעות יבוא בצורה טבעית בתוך הזרימה.'
}
